...
# Detect if this is an ETC test (has ETC-specific fork variables)
def is_etc_test:
  env.HIVE_FORK_ATLANTIS != null or
  env.HIVE_FORK_AGHARTA != null or
  env.HIVE_FORK_PHOENIX != null or
  env.HIVE_FORK_THANOS != null or
  env.HIVE_FORK_MAGNETO != null or
  env.HIVE_FORK_ECIP1017 != null
;

# Standard Ethash engine for ETH tests
def ethash_engine:
  {
    "Ethash": {
      "params": {
...
def etchash_engine:
  {
    "Etchash": {
      "params": {
        ...
        "homesteadTransition": env.HIVE_FORK_HOMESTEAD|to_hex,
        "eip100bTransition": env.HIVE_FORK_BYZANTIUM|to_hex,
        # ECIP-1099: DAG size reduction - required for Etchash
        "ecip1099Transition": (env.HIVE_FORK_THANOS|to_hex // "0x0"),
        # ECIP-1017: Monetary policy with 5M block eras
        "ecip1017EraRounds": "0x4c4b40",
        # ETC does NOT support DAO fork - leave out daoHardforkTransition
        "blockReward": {
          "0x0": "0x4563918244F40000",
          (env.HIVE_FORK_BYZANTIUM|to_hex//""): "0x29A2241AF62C0000",
          (env.HIVE_FORK_CONSTANTINOPLE|to_hex//""): "0x1BC16D674EC80000",
...
# Choose engine based on test type
def consensus_engine:
  if env.HIVE_CLIQUE_PERIOD then clique_engine
  elif is_etc_test then etchash_engine
  else ethash_engine
  end
;

{
  ...
  "params": {
    ...
    # Petersburg (also Agharta on ETC)
    ...
    # London/Spiral on ETC - NOTE: ETC Spiral does NOT include EIP-1559
    # For compatibility with ETH tests that expect EIP-1559, we include it
    # but ETC mainnet Spiral excludes it
    ...
    # Merge - ETC doesn't have the merge, but for test compatibility
    "MergeForkIdTransition": env.HIVE_MERGE_BLOCK_ID|to_hex,

    # Shanghai - ETC doesn't have Shanghai, but for test compatibility
    ...
    # Cancun - ETC doesn't have Cancun, but for test compatibility
